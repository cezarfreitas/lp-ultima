...
export default function AdminTestimonials() {
  ...
  const handleAddTestimonial = async () => {
    if (!newTestimonial.name || !newTestimonial.role || !newTestimonial.company || !newTestimonial.content) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setMessage({ type: 'error', text: errorData.error || 'Erro ao adicionar depoimento' });
      }
    } catch (error) {
  ...
  const handleUpdateTestimonial = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setMessage({ type: 'error', text: errorData.error || 'Erro ao atualizar depoimento' });
      }
    } catch (error) {
  ...
  const handleDeleteTestimonial = async (testimonialId: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setMessage({ type: 'error', text: errorData.error || 'Erro ao deletar depoimento' });
      }
    } catch (error) {
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange('max_testimonials', parseInt(e.target.value))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                  />
                </div>
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className={`rounded-lg p-8 ${
                formData.background_type === 'white' ? 'bg-white' :
                formData.background_type === 'gray' ? 'bg-gray-50' :
                ...
                {testimonials.slice(0, Math.min(3, formData.max_testimonials)).map((testimonial) => (
                  <div key={testimonial.id} className="bg-white rounded-lg shadow-sm p-6 mb-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  <ImageUploadCompressed 
                    ...
                    onUrlChange={(url) => setNewTestimonial({ ...newTestimonial, avatar_url: url })}
                    previewHeight="h-24"
                  />
                </div>
            ...
            <div className="bg-white shadow rounded-lg p-6">
                  ...
                  <div key={testimonial.id} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-gray-700 mb-3">"{testimonial.content}"</p>
                        <p className="text-xs text-gray-500">Posição: {testimonial.position}</p>
                      </div>
              ...
              {testimonials.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">Comece adicionando alguns depoimentos de clientes.</p>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <ImageUploadCompressed 
                    ...
                    onUrlChange={(url) => setEditingTestimonial({ ...editingTestimonial, avatar_url: url })}
                    previewHeight="h-24"
                  />
                </div>
                ...
                <div className="md:col-span-2">
                  <label className="flex items-center">
                    <input
                      ...
                      className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                    />
                    <span className="ml-2 text-sm text-gray-700">Depoimento ativo</span>
                  </label>
                </div>
              </div>
