...
export default function SetupComplete() {
  const [migrations, setMigrations] = useState<MigrationStatus[]>([
    {
      name: "Showroom",
      endpoint: "/api/migrate-showroom",
      status: "pending",
      message: "",
    },
    {
      name: "FAQ",
      endpoint: "/api/migrate-faq",
      status: "pending",
      message: "",
    },
    {
      name: "Testimonials",
      endpoint: "/api/migrate-testimonials",
      status: "pending",
      message: "",
    },
    {
      name: "About",
      endpoint: "/api/migrate-about",
      status: "pending",
      message: "",
    },
    {
      name: "Footer",
      endpoint: "/api/migrate-footer",
      status: "pending",
      message: "",
    },
    {
      name: "SEO",
      endpoint: "/api/migrate-seo",
      status: "pending",
      message: "",
    },
    {
      name: "Pixels",
      endpoint: "/api/migrate-pixels",
      status: "pending",
      message: "",
    },
  ]);
  ...
  const runAllMigrations = async () => {
    ...
    for (let i = 0; i < migrations.length; i++) {
      ...
      setMigrations((prev) =>
        prev.map((m, index) =>
          index === i
            ? { ...m, status: "running", message: "Executando..." }
            : m,
        ),
      );
        ...
        <div className="space-y-6">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
            ...
            <p className="text-blue-800 mb-4">
              Este processo executará todas as migrações necessárias para
              configurar completamente a aplicação, incluindo a nova otimização
              de SEO com foco em "Seja um Lojista Oficial Ecko".
            </p>
            <ul className="list-disc list-inside space-y-2 text-blue-800">
              ...
              <li>
                <strong>Migração do SEO</strong> - Otimizado para "Seja um
                Lojista Oficial Ecko"
              </li>
              <li>
                <strong>Migração dos Pixels (NOVO)</strong> - Sistema de
                rastreamento GA4, Meta Pixel, GTM
              </li>
            </ul>
          </div>
              ...
              {migrations.map((migration, index) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <span
                      className={`text-sm font-medium ${getStatusColor(migration.status)}`}
                    >
                    ...
                    {migration.message && (
                      <p
                        className={`text-xs mt-1 ${getStatusColor(migration.status)}`}
                      >
                        {migration.message}
                      </p>
                    )}
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-green-800">
                    Todas as migrações foram executadas com sucesso. A aplicação
                    está otimizada para SEO com foco em "Seja um Lojista Oficial
                    Ecko".
                  </p>
                  <div className="mt-4">
                    <a
