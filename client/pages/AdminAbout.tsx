...
export default function AdminAbout() {
  ...
  const handleAddStat = async () => {
    if (!newStat.title || !newStat.value || !newStat.icon) {
      setMessage({
        type: "error",
        text: "Título, valor e ícone são obrigatórios",
      });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({
          type: "success",
          text: "Estatística adicionada com sucesso!",
        });
      } else {
        const errorData = await response.json();
        setMessage({
          type: "error",
          text: errorData.error || "Erro ao adicionar estatística",
        });
      }
    } catch (error) {
  ...
  const handleUpdateStat = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({
          type: "success",
          text: "Estatística atualizada com sucesso!",
        });
      } else {
        const errorData = await response.json();
        setMessage({
          type: "error",
          text: errorData.error || "Erro ao atualizar estatística",
        });
      }
    } catch (error) {
  ...
  const handleDeleteStat = async (statId: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({
          type: "success",
          text: "Estatística deletada com sucesso!",
        });
      } else {
        const errorData = await response.json();
        setMessage({
          type: "error",
          text: errorData.error || "Erro ao deletar estatística",
        });
      }
    } catch (error) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="flex items-center mt-6">
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange("show_stats", e.target.checked)
                      }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("button_url", e.target.value)
                    }
                    placeholder="#form"
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                  />
                ...
                <div>
                  <label className="flex items-center mt-6">
                    <input
                      ...
                      onChange={(e) =>
                        setNewStat({ ...newStat, is_active: e.target.checked })
                      }
                      className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                    />
                    <span className="ml-2 text-sm text-gray-700">
