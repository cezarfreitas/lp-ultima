...
export default function APIHealthIndicator({ 
  ...
  const { isHealthy, isChecking, lastCheck, forceCheck, circuitStats } = useApiHealth();
  ...
  return (
    <div className={`${className}`}>
      <div className="flex items-center space-x-2">
        <div className={`w-2 h-2 bg-${healthColor}-400 rounded-full ${isChecking ? 'animate-pulse' : ''}`}></div>
        ...
        <div className="mt-2 text-xs text-gray-500">
          ...
          {!isHealthy && (
            <div className="mt-1 space-y-1">
              ...
              {Object.entries(circuitStats).map(([endpoint, stats]: [string, any]) => (
                <div key={endpoint} className="ml-2 flex justify-between">
                  <span>{endpoint}:</span>
                  <span className={`${stats.isOpen ? 'text-red-600' : 'text-green-600'}`}>
                    {stats.isOpen ? `Bloqueado (${stats.failureCount} falhas)` : 'Disponível'}
                  </span>
                </div>
              ))}
            </div>
