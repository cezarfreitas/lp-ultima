...
export const getAboutSection: RequestHandler = async (req, res) => {
  try {
    ...
    const [sectionRows] = await pool.execute('SELECT * FROM about_section LIMIT 1');
...
export const createAboutStat: RequestHandler = async (req, res) => {
  try {
    ...
    const { title, value, description, icon, is_active, position } = validation.data;
    
    // Get section ID
    const [sectionRows] = await pool.execute('SELECT id FROM about_section LIMIT 1');
    const section = (sectionRows as any[])[0];
    ...
    const [result] = await pool.execute(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [section.id, title, value, description || '', icon, is_active ?? true, finalPosition]);
    
    const insertId = (result as any).insertId;
    
    // Get the created stat
    const [rows] = await pool.execute('SELECT * FROM about_stats WHERE id = ?', [insertId]);
...
export const updateAboutStat: RequestHandler = async (req, res) => {
  try {
    ...
    // Return updated data
    const [rows] = await pool.execute('SELECT * FROM about_stats WHERE id = ?', [id]);
...
export const deleteAboutStat: RequestHandler = async (req, res) => {
  try {
    ...
    const [result] = await pool.execute('DELETE FROM about_stats WHERE id = ?', [id]);
