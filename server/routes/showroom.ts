...
export const createShowroomItem: RequestHandler = async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute('SELECT * FROM showroom_items WHERE id = ?', [insertId]);
...
export const updateShowroomItem: RequestHandler = async (req, res) => {
  try {
    ...
    // Return updated data
    const [rows] = await pool.execute('SELECT * FROM showroom_items WHERE id = ?', [id]);
...
export const deleteShowroomItem: RequestHandler = async (req, res) => {
  try {
    ...
    const [result] = await pool.execute('DELETE FROM showroom_items WHERE id = ?', [id]);
