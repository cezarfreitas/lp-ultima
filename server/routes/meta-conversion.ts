...
export async function sendMetaConversion(req: Request, res: Response) {
  try {
    const { pixel_id, event_name, user_data, event_time, custom_data } = req.body as ConversionPayload;
    ...
    if (!access_token) {
      return res.status(400).json({ error: 'Access token not found for this pixel' });
    }
    ...
    const eventData = {
      data: [{
        ...
        event_source_url: req.headers.referer || req.headers.origin || 'https://sejaum.lojista.ecko.com.br'
      }]
    };
    ...
    if (metaResponse.ok) {
      ...
      res.status(400).json({ error: 'Failed to send conversion', details: metaResult });
    }
