...
export const getDesignSettings: RequestHandler = async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM design_settings ORDER BY id DESC LIMIT 1');
...
export const createDesignSettings: RequestHandler = async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [primary_color, secondary_color, accent_color, background_color, text_color, font_family, font_size_base, font_weight_normal, font_weight_bold, border_radius]);
    
    const insertId = (result as any).insertId;
    
    // Return newly created data
    const [rows] = await pool.execute('SELECT * FROM design_settings WHERE id = ?', [insertId]);
