...
export const fixHeroUrls: RequestHandler = async (req, res) => {
  try {
    ...
    const [heroRows] = await pool.execute("SELECT * FROM hero_sections LIMIT 1");
    ...
    if (hero.logo_image && hero.logo_image.includes("/uploads/medium/") && !fs.existsSync(path.join(process.cwd(), hero.logo_image))) {
      ...
      const matchingFile = files.find(file => {
        ...
        return fileBaseName.includes(baseName.split('-')[0]) || baseName.includes(fileBaseName.split('-')[0]);
      });
    ...
    for (const field of imageFields) {
      if (hero[field] && hero[field].includes("/uploads/medium/") && !fs.existsSync(path.join(process.cwd(), hero[field]))) {
